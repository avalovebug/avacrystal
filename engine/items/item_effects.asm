...
.okay_1
	...
	ld a, 1
.statuscheck
; BUG FIXED: BRN/PSN/PAR do not affect catch rate (see docs/bugs_and_glitches.md)
	ld b, a
...
; BUG FIXED: Catching a Transformed Pokémon always catches a Ditto (see docs/bugs_and_glitches.md)
...
; BUG ADDRESSED: Love Ball boosts catch rate for the wrong gender (see docs/bugs_and_glitches.md)
	ld a, d
	pop de
	cp d
	pop bc
...
.loop
; BUG FIXED: Fast Ball only boosts catch rate for three Pokémon (see docs/bugs_and_glitches.md)
	ld a, BANK(FleeMons)
	call GetFarByte
